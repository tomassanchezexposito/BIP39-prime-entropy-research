...
# ---------------- Generación ordenada de direcciones desde archivo ----------

def candidate_from_tokens_ordered(tokens, valid_words):
    values = [normalize_mnemonic(x) for x in tokens if str(x).strip()]
    emitted = set()
    for value in values:
        parts = value.split()
        if len(parts) == 12 and all(w in valid_words for w in parts):
            phrase = " ".join(parts)
            if phrase not in emitted:
                emitted.add(phrase)
                yield phrase
    flat = []
    for value in values:
...
def iter_txt_ordered(path: Path, valid_words: set[str]):
...
def iter_csv_ordered(path: Path, valid_words: set[str]):
    with path.open('r', encoding='utf-8-sig', errors='replace', newline='') as f:
        sample = f.read(4096)
        f.seek(0)
        try:
            ...
            dialect = csv.excel
        for row in csv.reader(f, dialect):
            yield from candidate_from_tokens_ordered(row, valid_words)


def iter_xlsx_ordered(path: Path, valid_words: set[str]):
    NS = '{http://schemas.openxmlformats.org/spreadsheetml/2006/main}'
    with zipfile.ZipFile(path, 'r') as z:
        shared = []
        if 'xl/sharedStrings.xml' in z.namelist():
        ...
        for sheet_name in sheet_names:
            with z.open(sheet_name, 'r') as fh:
                        ...
                        if ctype == 'inlineStr':
                            ...
                            if v is not None and v.text is not None:
                                if ctype == 's':
                                    ...
                                    value = v.text
                        values.append(value)
                    yield from candidate_from_tokens_ordered(values, valid_words)
                    elem.clear()


def iter_candidate_phrases_ordered(path: Path, words: list[str]):
    valid_words = set(words)
    suffix = path.suffix.lower()
    if suffix in ('.txt', '.log', '.md'):
        yield from iter_txt_ordered(path, valid_words)
    elif suffix in ('.csv', '.tsv'):
        yield from iter_csv_ordered(path, valid_words)
    elif suffix == '.xlsx':
        yield from iter_xlsx_ordered(path, valid_words)
    else:
        raise ValueError('Formato no admitido. Utiliza TXT, CSV, TSV o XLSX.')


def count_candidate_phrases(path: Path, words: list[str]) -> int:
    return sum(1 for _ in iter_candidate_phrases_ordered(path, words))


def export_btc_addresses_ordered(input_path: Path, output_path: Path, words: list[str], passphrase: str, path: str, progress_cb=None, stop_event=None):
    total = count_candidate_phrases(input_path, words)
    processed = valid = invalid = 0
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with output_path.open('w', encoding='utf-8', buffering=1024 * 1024) as out:
        for phrase in iter_candidate_phrases_ordered(input_path, words):
            if stop_event is not None and stop_event.is_set():
                break
            processed += 1
            try:
                result = derive_btc_address(phrase, words, passphrase=passphrase, path=path)
                out.write(result['address'] + '\n')
                valid += 1
            except Exception as exc:
                msg = str(exc).replace('\n', ' ')
                out.write('ERROR_BIP39:' + msg + '\n')
                invalid += 1
            if progress_cb and (processed == 1 or processed % 25 == 0 or processed == total):
                progress_cb(processed, total, valid, invalid)
    return {
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title('BIP-39 / Bitcoin — generar direcciones desde archivo V3.0')
    ...
    def _build(self):
        ...
        ttk.Label(main, text='BIP-39 / Bitcoin — generar direcciones desde un archivo de frases', font=('Segoe UI', 17, 'bold')).pack(anchor='w')
        ttk.Label(main, text=('Carga un TXT/CSV/TSV/XLSX con frases BIP-39. La aplicación deriva una dirección Bitcoin Native SegWit para cada frase y crea un TXT con una dirección por línea en exactamente el mismo orden.'), wraplength=1080, justify='left').pack(anchor='w', pady=(5, 12))

        input_frame = ttk.LabelFrame(main, text='1. Archivo de frases BIP-39', padding=10)
        input_frame.pack(fill='x')
        ttk.Entry(input_frame, textvariable=self.input_var, state='readonly').pack(side='left', fill='x', expand=True)
        ttk.Button(input_frame, text='Seleccionar archivo…', command=self.select_input).pack(side='left', padx=(8, 0))
        ...
        ttk.Entry(params, textvariable=self.path_var, width=28).grid(row=0, column=1, padx=(8, 20), sticky='w')
        ttk.Label(params, text='Passphrase BIP-39:').grid(row=0, column=2, sticky='w')
        ttk.Entry(params, textvariable=self.pass_var, width=36, show='•').grid(row=0, column=3, padx=(8, 0), sticky='ew')
        params.columnconfigure(3, weight=1)

        output_frame = ttk.LabelFrame(main, text='3. Archivo TXT de direcciones Bitcoin', padding=10)
        output_frame.pack(fill='x', pady=(12, 0))
        ttk.Entry(output_frame, textvariable=self.output_var, state='readonly').pack(side='left', fill='x', expand=True)
        ttk.Button(output_frame, text='Elegir destino…', command=self.select_output).pack(side='left', padx=(8, 0))

        actions = ttk.Frame(main)
        actions.pack(fill='x', pady=(12, 0))
        self.generate_btn = ttk.Button(actions, text='Generar direcciones Bitcoin', command=self.start_export)
        self.generate_btn.pack(side='left')
        self.stop_btn = ttk.Button(actions, text='Detener', command=self.stop_export, state='disabled')
        ...
        manual = ttk.LabelFrame(main, text='4. Comprobar una frase individual', padding=10)
        manual.pack(fill='x', pady=(12, 0))
        self.mnemonic_text = tk.Text(manual, height=3, wrap='word')
        self.mnemonic_text.pack(fill='x')
        ttk.Button(manual, text='Calcular dirección Bitcoin', command=self.derive_single).pack(anchor='w', pady=(8, 0))
    ...
    def select_input(self):
        path = filedialog.askopenfilename(title='Selecciona el archivo con las frases', filetypes=[('Archivos admitidos', '*.txt *.csv *.tsv *.xlsx'), ('Texto', '*.txt'), ('CSV', '*.csv'), ('Excel', '*.xlsx'), ('Todos', '*.*')])
        if not path:
            return
        self.input_file = Path(path)
        self.input_var.set(str(self.input_file))
        suggested = self.input_file.parent / (self.input_file.stem + '_direcciones_bitcoin.txt')
        self.output_file = suggested
        self.output_var.set(str(suggested))
        self.status_var.set('Archivo de frases seleccionado.')

    def select_output(self):
        path = filedialog.asksaveasfilename(title='Guardar direcciones Bitcoin', defaultextension='.txt', filetypes=[('Archivo de texto', '*.txt'), ('Todos', '*.*')])
        if not path:
            return
    ...
    def start_export(self):
        ...
        if not self.input_file:
            messagebox.showwarning('Falta archivo', 'Selecciona primero el archivo de frases.')
            return
        if not self.output_file:
            messagebox.showwarning('Falta destino', 'Selecciona el archivo TXT de salida.')
            return
        try:
        ...
        self.status_var.set('Contando frases del archivo…')
        self.set_output('Procesamiento en curso…')
        input_path = self.input_file
        output_path = self.output_file
        passphrase = self.pass_var.get()
        derivation_path = self.path_var.get()

        def progress_cb(processed, total, valid, invalid):
            self.progress_queue.put(('progress', {'processed': processed, 'total': total, 'valid': valid, 'invalid': invalid}))

        def worker():
            try:
                stats = export_btc_addresses_ordered(input_path=input_path, output_path=output_path, words=self.words, passphrase=passphrase, path=derivation_path, progress_cb=progress_cb, stop_event=self.stop_event)
        ...
        try:
            while True:
                ...
                if kind == 'progress':
                    ...
                    self.status_var.set(f"Procesadas {processed}/{total} ({pct:.1f}%) · direcciones: {payload['valid']} · errores: {payload['invalid']}")
                elif kind == 'done':
                    self.running = False
                    self.generate_btn.configure(state='normal')
                    self.stop_btn.configure(state='disabled')
                    total = payload['total_detected']
                    self.progress['maximum'] = max(1, total)
                    self.progress['value'] = payload['processed']
                    self.status_var.set('Proceso detenido por el usuario.' if payload['stopped'] else 'Proceso finalizado.')
                    self.set_output(f"Archivo de entrada:\n{self.input_file}\n\nArchivo de salida:\n{payload['output_path']}\n\nFrases detectadas: {payload['total_detected']}\nProcesadas: {payload['processed']}\nDirecciones generadas: {payload['valid_addresses']}\nErrores BIP-39: {payload['errors']}\n\nCada línea de salida corresponde a la frase situada en la misma posición de la secuencia detectada en el archivo de entrada.")
                    if not payload['stopped']:
                        messagebox.showinfo('Proceso finalizado', 'Se ha creado el archivo de direcciones Bitcoin:\n\n' + payload['output_path'])
                elif kind == 'error':
                    self.running = False
                    self.generate_btn.configure(state='normal')
                    self.stop_btn.configure(state='disabled')
    ...
    def derive_single(self):
        ...
        try:
            result = derive_btc_address(mnemonic, self.words, passphrase=self.pass_var.get(), path=self.path_var.get())
            self.status_var.set('Dirección calculada correctamente')
            self.set_output(f"Frase:\n{result['mnemonic']}\n\nDirección Bitcoin:\n{result['address']}\n\nEntropía BIP-39:\n{result['entropy_hex']}\nChecksum: {result['checksum_bits']}\nRuta: {self.path_var.get()}")
        except Exception as exc:
