...
"""
Generador V2.4: serie infinita creciente de posiciones de primos + 24 palabras BIP-39.
...
Así, aunque la siguiente posición local sea menor que la anterior, su posición
absoluta y su primo absoluto siempre son mayores.

Ejemplo:
    anterior absoluta = 1830
    siguiente local    = 500
    siguiente absoluta = 2548  (500 + 2048)

ENTROPÍA
--------
Modelo BIP-39 de 24 palabras:
- 2048 = 2^11 símbolos.
- 24 símbolos = 264 bits codificados.
- 256 bits de entropía + 8 bits de checksum SHA-256.
- La posición local inicial fija los primeros 11 bits.
- Los 245 bits restantes proceden de secrets (CSPRNG del sistema operativo).
- Se aplica un filtro por rechazo de patrones lineales: órdenes ascendentes o descendentes
  con cualquier separación, progresiones de paso fijo y sus cruces del borde 2048↔1.

IMPORTANTE
----------
- La coordenada absoluta infinita NO aumenta la entropía de las 24 palabras:
  la frase sigue codificando 256 bits + checksum. El desplazamiento absoluto
  es una capa adicional de coordenadas.
- Se guarda sólo un hash SHA-256 de cada frase para evitar repetir localmente
  una frase ya emitida por esta instalación.
- Implementación experimental. No debe considerarse un generador de cartera
  auditado ni software de custodia.
"""
...
APP_NAME = "GeneradorPrimosPalabrasV24BIP39FiltroLinealTotal"
...
# ----------------------
# Se excluyen patrones que no se desean por su estructura lineal evidente.
#
# 1) ORDEN LINEAL CON CUALQUIER ESPACIO:
#    - 24 posiciones estrictamente ascendentes dentro de 1..2048.
#    - 24 posiciones estrictamente descendentes dentro de 1..2048.
#    - también si el recorrido ordenado cruza UNA vez el borde 2048 <-> 1.
#
#    Ejemplos:
#      1,2,3,...,12
#      1,3,8,25,...          (espacios desiguales)
#      2040,2045,2,7,...     (ascendente con un cruce)
#
# 2) PROGRESIÓN ARITMÉTICA MODULAR:
#    cualquier paso fijo, positivo o negativo, aunque dé varias vueltas
#    sobre el bloque de 2048 símbolos:
#      1,3,5,7,...           paso +2
#      2046,2048,2,4,...     paso +2 cruzando el borde
#      100,97,94,91,...      paso -3
#
# 3) CASO DEGENERADO:
#    24 veces la misma posición también se descarta.
#
# IMPORTANTE:
# El filtro se aplica a las POSICIONES LOCALES 1..2048. La serie absoluta
# continúa siendo estrictamente creciente por construcción.


def _forward_distance(a: int, b: int) -> int:
    """Distancia positiva hacia delante sobre el círculo 1..2048."""
    return (b - a) % BLOCK


def _backward_distance(a: int, b: int) -> int:
    """Distancia positiva hacia atrás sobre el círculo 1..2048."""
    return (a - b) % BLOCK


def is_fixed_step_progression(local_positions: list[int]) -> bool:
    """Detecta una progresión de 24 posiciones con paso fijo módulo 2048, incluido paso 0."""
...
def is_ordered_with_arbitrary_gaps(local_positions: list[int]) -> bool:
    """
    Detecta un recorrido lineal ASC o DESC con cualquier separación.

    Se permite como máximo un cruce del borde del bloque. Equivalentemente:
    al avanzar en una sola dirección, la distancia total recorrida debe ser
    menor que 2048. Esto evita clasificar como "lineal" cualquier secuencia
    aleatoria que dé múltiples vueltas alrededor del bloque.
    """
    if len(local_positions) != WORD_COUNT:
        return False

    # Debe tratarse de 12 posiciones distintas para un orden estricto.
...
def is_forbidden_linear_pattern(local_positions: list[int]) -> bool:
    """Filtro maestro de patrones lineales no admitidos."""
    if len(local_positions) != WORD_COUNT:
        return False

    # Incluye 12 iguales y cualquier progresión de paso fijo.
    if is_fixed_step_progression(local_positions):
        return True

    # Incluye orden ASC/DESC con separaciones arbitrarias, con hasta un cruce.
...
def lift_local_series(first_absolute: int, local_positions: list[int]) -> list[int]:
    if len(local_positions) != WORD_COUNT:
        raise ValueError(f"Se esperaban {WORD_COUNT} posiciones locales.")
...
def checksum8(entropy_bytes: bytes) -> int:
    """Primeros 8 bits de SHA-256 para ENT=256, como exige BIP-39."""
    return hashlib.sha256(entropy_bytes).digest()[0]


def indexes_from_entropy(entropy_int: int) -> tuple[list[int], int]:
    """Convierte 256 bits de entropía en 24 índices BIP-39 de 11 bits."""
    entropy_bytes = entropy_int.to_bytes(32, "big")
    cs = checksum8(entropy_bytes)
    combined = (entropy_int << 8) | cs
    indexes = [
        (combined >> (11 * (WORD_COUNT - 1 - i))) & 0x7FF
...
def phrase_is_valid(indexes: list[int]) -> bool:
    """Valida estructura BIP-39 de 24 palabras: 256 bits + checksum de 8 bits."""
    ...
    entropy_bytes = entropy_int.to_bytes(32, "big")
    return cs == checksum8(entropy_bytes)


# ---------------------------------------------------------------------------
# Cálculo exacto del primo impar que ocupa una posición absoluta g.
# Posición absoluta 1 -> 3; 2 -> 5; 3 -> 7; etc.
# Se usa una criba segmentada para mantener la memoria acotada.
# ---------------------------------------------------------------------------
...
def upper_bound_for_odd_prime_position(n: int) -> int:
    ...
    # El primo impar n es el primo ordinario número n+1 (porque se excluye 2).
    m = n + 1
    small = [2, 3, 5, 7, 11, 13]
    if m < len(small):
        return small[m] + 10
...
def odd_primes_at_positions(target_positions: list[int], progress=None) -> dict[int, int]:
    """
    Devuelve {posición_absoluta_de_primo_impar: primo} para objetivos positivos.
    Usa segmentos de impares y memoria acotada.
    """
    ...
    while True:
        ...
        found: dict[int, int] = {}
        ordinal = 0  # ordinal dentro de los primos impares: 3 ocupa 1

        segment_span = 1_000_000  # amplitud numérica; sólo almacenamos impares
        low = 3
        while low <= upper and len(found) < len(target_set):
            ...
            for p in base_primes:
                ...
                if start > high:
                    continue
                idx = (start - low) // 2
                step = p  # en índices de impares, +2p equivale a +p índices
        ...
        # Cota insuficiente (caso muy raro): ampliar y repetir.
...
def build_candidate(first_absolute: int, items: list[Item]) -> dict:
    ...
    # Generamos una frase no repetida localmente.
    con = db_connect()
    try:
        while True:
            random_tail = secrets.randbits(245)
            entropy_int = (first_index << 245) | random_tail
            ...
            # FILTRO V2.3: excluir cualquier patrón lineal estructurado:
            # orden ASC/DESC con cualquier separación o progresión de paso fijo.
    ...
    return {
        ...
        "entropy_hex": entropy_int.to_bytes(32, "big").hex(),
        "checksum_bits": f"{cs:08b}",
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Generador V2.4 — 24 palabras BIP-39 · filtro lineal total")
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            main,
            text="Generador V2.4 — 24 palabras BIP-39 · serie infinita creciente",
        ...
        explanation = ttk.Label(
            main,
            text=(
                "Introduce una posición ABSOLUTA positiva de la sucesión de primos impares. "
                "Las 2048 posiciones base se repiten como símbolos por bloques: la palabra usa "
                "la posición local 1–2048, mientras que la posición absoluta siempre avanza. "
                "Si el siguiente valor local es inferior, se toma su primera aparición en un bloque posterior. "
                "Además, se rechaza cualquier patrón lineal de 24 posiciones: orden ascendente o descendente "
                "con cualquier separación, progresiones de paso fijo y patrones que crucen el borde 2048↔1."
            ),
            wraplength=1135,
            justify="left"
        )
        ...
        self.generate_btn = ttk.Button(input_frame, text="Generar 24 palabras", command=self.on_generate)
        ...
        phrase_box = ttk.LabelFrame(main, text="Frase BIP-39 de 24 palabras", padding=10)
        ...
        note = ttk.Label(
            main,
            text=(
                "Regla infinita: gₙ₊₁ es la menor posición absoluta > gₙ que tenga la posición local "
                "generada por la entropía. Por ello las posiciones absolutas y sus primos son siempre "
                "estrictamente crecientes. La capa absoluta no añade bits a la frase de 24 palabras. "
                "Para posiciones absolutas muy grandes, calcular el primo exacto puede tardar más tiempo. "
                "Filtro lineal total activo: se regeneran automáticamente los patrones ordenados o progresivos."
            ),
            wraplength=1135,
            justify="left"
        )
    ...
    def on_generate(self):
        ...
        def progress(done, total):
            # Actualización ligera y segura desde el hilo de trabajo.
            if total > 0:
        ...
        def worker():
            try:
                ...
                candidate["absolute_primes"] = [primes_map[g] for g in candidate["absolute_positions"]]
    ...
    def _display_result(self, result: dict):
        ...
        self.status_var.set("Listo — creciente · filtro lineal total activo")
        self.generating = False
        self.generate_btn.configure(state="normal")
    ...
    def copy_phrase(self):
        ...
        self.update()
        messagebox.showinfo("Copiado", "La frase de 24 palabras se ha copiado al portapapeles.")
