...
"""
Generador V2.3-A24 — ancla posicional bidireccional para 24 palabras BIP-39.

OBJETIVO
--------
La variante V2.3-A de 12 palabras permite fijar la palabra derivada de una
posición absoluta en cualquier slot del grupo. Esta versión generaliza el mismo
modelo a grupos BIP-39 de 24 palabras.

Ejemplo conceptual:
    posición absoluta ancla = g
    posición dentro del grupo = 17

La palabra de g debe aparecer como palabra #17. Las posiciones absolutas #1..#16
se levantan HACIA ATRÁS (positivas y estrictamente menores que g) y las #18..#24
se levantan HACIA DELANTE (estrictamente mayores que g).
...
BIP-39 Y ENTROPÍA
-----------------
24 palabras codifican 264 bits: 256 bits de entropía + 8 bits de checksum.

- Si la palabra ancla ocupa una posición 1..23, sus 11 bits pertenecen por
  completo al campo de entropía. Se fijan exactamente esos 11 bits y los otros
  245 bits proceden de secrets.randbits(245).

- Si la palabra ancla ocupa la posición 24, el índice de 11 bits contiene
  3 bits de entropía + 8 bits de checksum. Se fijan los 3 bits de entropía
  correspondientes y se generan los otros 253 bits con CSPRNG; se usa rechazo
  hasta que el primer byte de SHA-256 coincida con los 8 bits finales del índice
  solicitado. La probabilidad esperada de aceptación por intento es ~1/256.

IMPORTANTE:
- La coordenada absoluta, los primos, el filtro estructural y SQLite NO añaden
  entropía criptográfica.
- El filtro V2.3 se generaliza al grupo completo de 24 posiciones y sigue siendo
  una regla de rechazo determinista.
- SQLite evita repetir LOCALMENTE frases ya registradas por esta instalación.
- Se usa una base SQLite separada de la versión de 12 palabras para no mezclar
  historiales ni contadores de espacios de frase distintos.
...
APP_NAME = "GeneradorPrimosPalabrasV23A24AnclaPosicional"
...
ENTROPY_BITS = 256
CHECKSUM_BITS = 8
MAX_CHECKSUM_ATTEMPTS = 5_000_000
...
def lift_local_series_anchored(
    ...
    """
    Levanta 24 posiciones locales a coordenadas absolutas estrictamente crecientes,
    dejando anchor_absolute exactamente en anchor_slot (1..24).
    ...
    """
    if len(local_positions) != WORD_COUNT:
        raise ValueError("Se esperaban 24 posiciones locales.")
    if not 1 <= anchor_slot <= WORD_COUNT:
        raise ValueError("La posición de la palabra ancla debe estar entre 1 y 24.")
...
def checksum8(entropy_bytes: bytes) -> int:
    """Checksum BIP-39 de 8 bits para ENT=256."""
    if len(entropy_bytes) != 32:
        raise ValueError("Para 24 palabras la entropía debe ocupar exactamente 32 bytes.")
...
def indexes_from_entropy(entropy_int: int) -> tuple[list[int], int]:
    if not 0 <= entropy_int < (1 << ENTROPY_BITS):
        raise ValueError("La entropía debe caber exactamente en 256 bits.")
...
def insert_fixed_11_bits(random245: int, target_index: int, word_slot: int) -> int:
    """
    Construye 256 bits fijando exactamente los 11 bits correspondientes a
    word_slot 1..23 y distribuyendo los 245 bits aleatorios en el resto.

    Se generan directamente los 245 bits libres: no se generan 256 bits para
    sobrescribir después una parte del valor.
    """
    if not 1 <= word_slot <= WORD_COUNT - 1:
        raise ValueError("Esta función sólo admite posiciones 1..23.")
    if not 0 <= target_index < BLOCK:
        raise ValueError("Índice BIP-39 fuera de 0..2047.")
    free_bits = ENTROPY_BITS - 11
    if not 0 <= random245 < (1 << free_bits):
        raise ValueError("random245 debe contener como máximo 245 bits.")

    shift = ENTROPY_BITS - 11 * word_slot
    low_width = shift
    low_mask = (1 << low_width) - 1 if low_width else 0
    low = random245 & low_mask
    high = random245 >> low_width
...
def entropy_with_forced_word(
    ...
    Retorna:
        (entropy_int, indexes, checksum8, intentos_checksum)

    Slots 1..23:
        11 bits del índice están dentro de ENT -> 245 bits CSPRNG restantes.

    Slot 24:
        3 bits del índice están en ENT y 8 son checksum. Se generan 253 bits
        CSPRNG por candidato y se rechaza hasta que el byte SHA-256 coincida.
    """
    if not 0 <= target_index < BLOCK:
        raise ValueError("Índice BIP-39 fuera de 0..2047.")
    if not 1 <= word_slot <= WORD_COUNT:
        raise ValueError("La posición de palabra debe estar entre 1 y 24.")

    if word_slot <= WORD_COUNT - 1:
        free_bits = ENTROPY_BITS - 11  # 245
        random_free = randbits(free_bits)
        if not 0 <= random_free < (1 << free_bits):
            random_free &= (1 << free_bits) - 1
        entropy_int = insert_fixed_11_bits(random_free, target_index, word_slot)
    ...
    # Palabra 24 = 3 bits finales de ENT + 8 bits checksum.
    target_entropy3 = target_index >> CHECKSUM_BITS
    desired_checksum = target_index & ((1 << CHECKSUM_BITS) - 1)
    free_bits = ENTROPY_BITS - 3  # 253

    for attempt in range(1, MAX_CHECKSUM_ATTEMPTS + 1):
        random253 = randbits(free_bits)
        if not 0 <= random253 < (1 << free_bits):
            random253 &= (1 << free_bits) - 1
        entropy_int = (random253 << 3) | target_entropy3
        indexes, cs = indexes_from_entropy(entropy_int)
        if cs == desired_checksum:
            if indexes[WORD_COUNT - 1] != target_index:
                raise RuntimeError("Error interno al fijar la palabra 24.")
...
def entropy_model_text(word_slot: int) -> str:
    if 1 <= word_slot <= WORD_COUNT - 1:
        return "11 bits fijados por el ancla + 245 bits CSPRNG"
    if word_slot == WORD_COUNT:
        return "3 bits fijados + 253 bits CSPRNG por candidato + checksum 8 bits por rechazo"
    return ""


# ---------------------------------------------------------------------------
# SQLite: historial separado para el espacio de 24 palabras
# ---------------------------------------------------------------------------
...
def db_connect() -> sqlite3.Connection:
    ...
    # El esquema conserva los campos de ancla de V2.3-A, pero esta versión usa
    # un APP_NAME distinto para no mezclar historial de 12 y 24 palabras.
...
def build_candidate(
    ...
    if not 1 <= anchor_slot <= WORD_COUNT:
        raise ValueError("La posición dentro del grupo debe estar entre 1 y 24.")
    ...
    # Con el filtro V2.3, si el ancla es la palabra 24 y está en el primer
    # bloque, las 24 absolutas quedarían 1..2048 en orden local estricto,
    # patrón que el filtro excluye por diseño.
    if anchor_slot == WORD_COUNT and anchor_absolute <= BLOCK:
        raise ValueError(
            "Con el filtro lineal V2.3 activo, una ancla en la palabra 24 dentro "
            "del primer bloque (1..2048) no puede producir una serie admitida: "
            "las 24 posiciones locales quedarían estrictamente ascendentes. "
    ...
    try:
        for candidate_attempt in range(1, MAX_CANDIDATE_ATTEMPTS + 1):
            ...
            return {
                ...
                "entropy_hex": entropy_int.to_bytes(ENTROPY_BITS // 8, "big").hex(),
                "checksum_bits": f"{cs:0{CHECKSUM_BITS}b}",
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Generador V2.3-A24 — ancla posicional bidireccional · 24 palabras")
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            main,
            text="Generador V2.3-A24 — palabra ancla en posición 1..24",
        ...
        ttk.Label(
            ...
            text=(
                ...
                "por su posición local 1..2048. Elige además en qué lugar del grupo (1..24) debe aparecer. "
        ...
        self.generate_btn = ttk.Button(
            input_frame, text="Generar 24 palabras", command=self.on_generate
        )
        ...
        phrase_box = ttk.LabelFrame(main, text="Serie de 24 palabras", padding=10)
    ...
    def _update_anchor_info(self):
        ...
        if g is None:
            ...
            return
        if slot is None:
            self.anchor_info_var.set("Selecciona una posición de palabra entre 1 y 24.")
            self.entropy_model_var.set("")
            return
    ...
    def on_generate(self):
        ...
        if anchor_slot is None:
            messagebox.showwarning(
                "Posición inválida", "Selecciona una posición dentro del grupo entre 1 y 24."
            )
            return
    ...
    def copy_phrase(self):
        ...
        self.update()
        messagebox.showinfo("Copiado", "La serie de 24 palabras se ha copiado al portapapeles.")
